...
def sendjstatresult(host,jstat,websugarurl,javaversion,javapath):
    hostname, weblogicname, weblogicversion = host.split('|')
    sendhandle = sendto.sendtowebsugar(websugarurl)
    if jstat[0] and jstat[1]:
        sendhandle.send(hostname, 'weblogicjavastatus', 1)
    else:
        sendhandle.send(hostname, 'weblogicjavastatus', 0)
    sendhandle.send(hostname, 'weblogicversion',weblogicversion)
    sendhandle.send(hostname,'weblogicname',weblogicname)
    sendhandle.send(hostname,'weblogicjavaversion',javaversion)
    sendhandle.send(hostname,'weblogicjavapath',javapath)
    keys = jstat[0].split()
    values = jstat[1].split()
    for i in xrange(len(keys)):
        key = 'weblogicgc_{}'.format(keys[i])
...
def checkjava(conf_path):
    ...
    conf.read(conf_path)
    hosts=conf.get('checkWeblogicServers','host').split(',')
    jpsname = conf.get('checkWeblogicServers', 'jpsname').split(',')
    websugarurl=conf.get('websugar','url')
    try:
        user = conf.get('checkWeblogicServers', 'user')
        passwd = conf.get('checkWeblogicServers', 'passwd')
        ips = conf.get('checkWeblogicServers', 'ip').split(',')
        port = int(conf.get('checkWeblogicServers', 'port'))
    except:
